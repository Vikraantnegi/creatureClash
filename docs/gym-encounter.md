...
> Phase 7 adds public levels, participant XP, persistent settlement and post-exchange journal training. The AI now exchanges only for an improvement in total trained stats, otherwise declining. See [current progression rules and verification](phase-7-verification.md). Earlier references below to no XP/level field or first-participant exchanges describe the original checkpoint.
